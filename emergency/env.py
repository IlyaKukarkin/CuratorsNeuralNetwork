...
import numpy as np
from emergency.calculate_reward import calculate_emergency_reward
...
class EmergencyCuratorEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, df):
        super(EmergencyCuratorEnv, self).__init__()
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(3,), dtype=np.float16)
    ...
    def step(self, action):
        ...
        reward = calculate_emergency_reward(df=self.df.iloc[self.current_day * 96 + self.current_step], action=action)
        ...
        if self.current_step == 96:
            ...
            # reward = calculate_emergency_reward(df=self.df.loc[get_curr_position: get_curr_position + 95], actions=self.actions)
